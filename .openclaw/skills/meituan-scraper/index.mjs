/**
 * meituan-scraper skill 钩子：调用前检查 supercrawler MCP 是否就绪；
 * 调用后识别风控错误码发出预警日志。
 *
...
export async function onBeforeInvoke({ toolName }) {
  ...
  try {
    ...
    if (!res.ok) {
      console.error(`[meituan-scraper] ⚠️  health 返回 ${res.status}，MCP stdio 仍可工作但 storage_peek HTTP 不可用`);
    }
  } catch {
...
export async function onAfterInvoke({ toolName, result }) {
  ...
  // 关键失败码预警
  if (/RATE_LIMITED/.test(txt)) {
    console.error(`[meituan-scraper] 🚨 风控触发，建议退避 5+ 分钟停止调用：${txt.slice(0, 200)}`);
  } else if (/LOGIN_REQUIRED|LOGIN_TIMEOUT/.test(txt)) {
    console.error(`[meituan-scraper] 🔐 登录态失效，需 auth_login：${txt.slice(0, 200)}`);
  }
}
