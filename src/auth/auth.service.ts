...
import { getCloakBrowserPath } from '../common/utils/cloakbrowser-path.util';
...
export class AuthService {
  ...
  /**
   * 确保浏览器已下载并就绪
   * 使用 CloakBrowser 官方 API 检查和准备浏览器
   */
  private async ensureBrowserReady(): Promise<void> {
    ...
    try {
      ...
      // 使用 CloakBrowser 官方 API 获取浏览器信息
      const info = await cloak.binaryInfo();
      
      this.logger.log(`📦 浏览器版本: ${info.version}`);
      this.logger.log(`💻 平台: ${info.platform}`);
      this.logger.log(`📂 缓存目录: ${info.cacheDir}`);
      this.logger.log(`✅ 已安装: ${info.installed}`);
      
      if (info.installed) {
        this.logger.log('✅ Chromium 浏览器已就绪，跳过下载');
        return;
      }

      // 浏览器未安装，使用 CloakBrowser 自动下载
      this.logger.warn('⚠️  Chromium 浏览器未下载，正在自动下载...');
      this.logger.warn('📦 预计大小: ~200MB | ⏱️  预计时间: 2-5 分钟（取决于网络速度）');
      this.logger.warn('💡 提示: 下载期间请勿关闭服务，下载完成后会自动继续');
      
      // 使用 CloakBrowser 的 ensureBinary 方法下载
      await cloak.ensureBinary();
      
      // 再次检查
      const infoAfter = await cloak.binaryInfo();
      if (infoAfter.installed) {
        this.logger.log('✅ Chromium 浏览器下载完成并就绪');
        return;
      }
      
      throw new Error('浏览器下载失败，请检查网络连接');
    } catch (error) {
      this.logger.error(`浏览器检查/下载失败: ${error.message}`);
      throw error;
    }
  }
